...
DragableTabArea::DragableTabArea(QWidget *parent) : QScrollArea(parent) {
  connect(this, SIGNAL(signalTabGroupCreated(DragableTabGroup *)), this,
          SLOT(slotTabGroupCreated(DragableTabGroup *)));
 ...
 */
DragableTabArea::~DragableTabArea() { main_layout = nullptr; }
 ...
 * （不一定在这个标签组里面）
 */
QBoxLayout *DragableTabArea::getGroupLayout(QBoxLayout *layout,
                                            DragableTabGroup *group) {
 ...
 * 尝试的时候如栈，若没有成功则出栈
 */
QBoxLayout *DragableTabArea::getGroupLayoutPath(QList<QBoxLayout *> &path,
                                                QBoxLayout *layout,
                                                DragableTabGroup *group) {
 ...
 * @return 标签组指针
 */
DragableTabGroup *DragableTabArea::createMainTabGroup(QWidget *widget,
                                                      QString label) {
 ...
 * @return 标签组指针
 */
DragableTabGroup *DragableTabArea::createTabGroup(QWidget *widget,
                                                  QString label) {
 ...
 */
DragableTabGroup *
DragableTabArea::splitGroupLayout(DragableTabGroup *base,
 ...
 */
DragableTabGroup *
DragableTabArea::splitGroupTab(DragableTabGroup *group, int index,
                               QBoxLayout::Direction direction, bool copy) {
  ...
  if (index > -1) {
    ...
    new_group->addTab(group->currentWidget(),
                      group->tabText(group->currentIndex()));
  }
 ...
 * @return 标签组（新窗口）指针
 */
DragableTabGroup *DragableTabArea::createTabWindow(QWidget *widget,
                                                   QString label) {
 ...
 * 标签组的某一标签移动到新窗口
 */
DragableTabGroup *DragableTabArea::createTabWindow(DragableTabGroup *group,
                                                   int index) {
  if (group == nullptr)
    return nullptr;
  if (index == -1)
 ...
 * 获取所有标签组的数量
 */
int DragableTabArea::count() { return tab_groups.count(); }
...
DragableTabGroup *DragableTabArea::mergeGroup(DragableTabGroup *group) {
  ...
  while (path.size()) {
    ...
    // 判断layout的数量，2个才能合并
    if (layout->count() > 1) {
      int index = (prev_layout == nullptr ? layout->indexOf(group)
      ...
      if (index > -1) {
        ...
        for (int i = index - 1; i >= 0; i--) {
          ...
          if (lay) {
            auto gro =
                qobject_cast<DragableTabGroup *>(lay->itemAt(0)->widget());
            if (gro)
              return mergeGroup(group, gro);
          }
        }
 ...
 * 合并标签组：dead => live
 */
DragableTabGroup *DragableTabArea::mergeGroup(DragableTabGroup *dead,
                                              DragableTabGroup *live) {
 ...
 */
QList<QBoxLayout *>
DragableTabArea::getGroupLayoutPath(DragableTabGroup *group) {
...
void DragableTabArea::groupCreateEvent(DragableTabGroup *group) {
  connect(group, &DragableTabGroup::signalNewTabWindowCreated, this,
          [=](DragableTabGroup *group) { groupCreateEvent(group); });
  ...
  connect(group, &DragableTabGroup::signalJsonToWidget, this,
          [=](QJsonObject object) { jsonToWidget(group, object); });
}
 ...
 */
QBoxLayout *
DragableTabArea::removeGroupUpperEmptyLayouts(DragableTabGroup *group) {
...
void DragableTabArea::slotTabGroupCreated(DragableTabGroup *group) {
  ...
  connect(group, SIGNAL(signalNewTabWindowCreated(DragableTabGroup *)), this,
          SLOT(slotTabGroupCreated(DragableTabGroup *)));
  ...
  connect(group, &DragableTabGroup::signalWidgetFocused, this,
          [=](QWidget *) { current_group = group; });

  connect(group, &DragableTabGroup::signalSplitCurrentTab, this,
          [=](QBoxLayout::Direction direction, bool copy) {
 ...
 * 创建新的标签组
 */
void DragableTabArea::slotTabGroupSplited(DragableTabGroup *base,
                                          QBoxLayout::Direction direction) {
...
void DragableTabArea::jsonToGroup(QBoxLayout *layout, QJsonObject object) {
  DragableTabGroup *group =
      static_cast<DragableTabGroup *>(createMainTabGroup());
 ...
 */
void DragableTabArea::jsonToWidget(DragableTabGroup *group,
                                   QJsonObject object) {
  Q_UNUSED(group)
  Q_UNUSED(object)
}
...
void DragableTabArea::fromJsonString(QString s) {
  ...
  while (!tab_groups.empty()) {
    DragableTabGroup *group =
        static_cast<DragableTabGroup *>(tab_groups.takeFirst());
...
void DragableTabArea::restoreClosedTab() { currentGroup()->restoreClosedTab(); }
